...
export async function collectBusinessSignals(url) {
  ...
  if (pageCheck.success) {
    ...
    const supportEmail =
      emails.find(
        email => email.includes('support') || email.includes('help') || email.includes('contact')
      ) || emails[0];
    ...
    const hasCareers = links.some(
      link =>
        link.href &&
        careerKeywords.some(
          keyword => link.href.toLowerCase().includes(keyword) || link.text.includes(keyword)
        )
    );
